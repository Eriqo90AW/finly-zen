    ...
    <div class="space-y-6 animate-fade-in-up">
      ...
      <div 
        class={`premium-card p-6 bg-white transition-all duration-500 overflow-hidden ${
          isChartVisible() ? "h-[160px] opacity-100" : "h-0 opacity-0 p-0 border-none"
        }`}
      >
        <div class="flex items-center justify-between mb-2">
           <p class="text-[10px] font-bold text-earth uppercase tracking-widest">Spend Velocity</p>
           <p class="text-xs font-outfit font-bold text-forest">$1,240 this period</p>
        </div>
        <div class="h-[100px]">
           <SolidApexCharts options={areaChartOptions} series={[{ name: 'Spent', data: [31, 40, 28, 51, 42, 109, 100] }]} type="area" height="100%" />
        </div>
      </div>
          ...
          {([date, items]) => (
            <div class="space-y-4">
              <div class="sticky top-0 bg-page-bg/80 backdrop-blur-md py-2 z-10 flex items-center justify-between border-b border-forest/5">
                <h4 class="text-sm font-outfit font-bold text-forest">{date}</h4>
                <p class="text-xs font-outfit text-earth font-medium">
                  Total: <span class="text-forest">${items.reduce((sum, i) => sum + i.amount, 0).toFixed(2)}</span>
                </p>
              </div>
              <div class="space-y-2">
                <For each={items}>
                  {(t) => (
                    <div class="premium-card p-4 flex items-center justify-between group cursor-pointer hover:border-spring">
                      <div class="flex items-center gap-4">
                        ...
                        <div>
                          <p class="font-outfit font-semibold text-forest leading-none">{t.merchant}</p>
                          <p class="text-xs text-earth mt-1">{t.note || t.category}</p>
                        </div>
                      </div>
                      <div class="text-right">
                        <p class="font-outfit font-bold text-forest">${t.amount.toFixed(2)}</p>
                        <p class="text-[10px] text-earth uppercase tracking-widest">Confirmed</p>
                      </div>
                    </div>
                  )}
                </For>
        ...
        {state.transactions.length === 0 && (
           <div class="p-20 text-center space-y-4">
              <div class="w-20 h-20 bg-sage/20 rounded-full flex items-center justify-center mx-auto">
                 <span class="material-icons text-4xl text-forest/20">history</span>
              </div>
              <div class="space-y-1">
                 <h3 class="text-xl font-cormorant text-forest">No transactions found</h3>
                 <p class="text-sm text-earth">Start tracking your spending to see it here.</p>
              </div>
           </div>
        )}
      </div>
