...
export const EstimatesTable = (props: EstimatesTableProps) => {
  ...
  const columns = [
    { label: "Current Quarter", data: estimates.current_quarter, period: "Jun 2026" },
    { label: "Next Quarter", data: estimates.next_quarter, period: "Sep 2026" },
    { label: "Current Year", data: estimates.current_year, period: "Dec 2026" },
    { label: "Next Year", data: estimates.next_year, period: "Dec 2027" },
  ];
      ...
      <td class="py-4 pl-4 pr-2">
        <div class="flex flex-col">
          ...
          <Show when={props.subLabel}>
            <span class="text-[10px] text-earth/60 uppercase tracking-tight">{props.subLabel}</span>
          </Show>
        </div>
      </td>
      <For each={columns}>
        {(col) => (
          <td class="py-4 px-4 text-sm font-outfit text-forest font-medium">
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody>
            ...
            <Row 
              ...
              render={(est) => (
                <div class="flex items-center gap-2">
                  <span class={`px-1.5 py-0.5 rounded text-[10px] font-bold ${est.eps_revisions_up_7d > 0 ? "bg-[#10B981]/10 text-[#10B981]" : "bg-earth/5 text-earth/40"}`}>
                    {est.eps_revisions_up_7d}↑
                  </span>
                  <span class="text-earth/20 text-[10px]">/</span>
                  <span class={`px-1.5 py-0.5 rounded text-[10px] font-bold ${est.eps_revisions_down_30d > 0 ? "bg-[#F43F5E]/10 text-[#F43F5E]" : "bg-earth/5 text-earth/40"}`}>
                    {est.eps_revisions_up_7d}↓
                  </span>
                </div>
              )} 
            ...
            <Row 
              ...
              render={(est) => (
                <div class="flex items-center gap-2">
                  <span class={`px-1.5 py-0.5 rounded text-[10px] font-bold ${est.eps_revisions_up_30d > 0 ? "bg-[#10B981]/10 text-[#10B981]" : "bg-earth/5 text-earth/40"}`}>
                    {est.eps_revisions_up_30d}↑
                  </span>
                  <span class="text-earth/20 text-[10px]">/</span>
                  <span class={`px-1.5 py-0.5 rounded text-[10px] font-bold ${est.eps_revisions_down_30d > 0 ? "bg-[#F43F5E]/10 text-[#F43F5E]" : "bg-earth/5 text-earth/40"}`}>
                    {est.eps_revisions_down_30d}↓
                  </span>
                </div>
              )} 
            />
            ...
            <tr class="bg-forest/5">
              ...
              <For each={columns}>
                {(col) => (
                  <td class="py-3 px-4 text-xs font-bold text-forest">
